...
def forecast(config, records=None):
    ...
    for day in range(1,15):
        ...
        if len(matches) < 2:
            ...
            fallback = True
        baseline = (sum(matches)+len(matches)-1)//len(matches)
        uplift = config['uplift'] if config['event_start'] <= day < config['event_start']+config['event_duration'] and config['scenario'] != 'Normal trading' else 0
        units = int((Decimal(baseline)*(1+Decimal(str(config['owner_adjustment']))/100)*(1+Decimal(str(uplift))/100)).to_integral_value(rounding=ROUND_CEILING))
        daily.append(dict(day=day, date=target.isoformat(), baseline=baseline, demand=units))
    safety = int((Decimal(sum(r['demand'] for r in daily))*Decimal(str(config['buffer_days']))/14).to_integral_value(rounding=ROUND_CEILING))
    ...
    if quantity==0:
        ...
        for a,b in itertools.combinations(eligible,2):
            for qa in range(a['minimum'], min(a['capacity'],quantity-b['minimum'])+1):
                ...
                if c: candidates.append(c)
    if not candidates:
        if quantity == 0 and urgent_quantity:
            reason = 'The net purchase quantity is zero, but an urgent new purchase was requested. Reconcile the urgent requirement with existing and incoming stock.'
        elif quantity == 0:
            reason = 'No net purchase is needed, but delivery timing leaves a stock gap. Review incoming arrivals or arrange an emergency transfer.'
        else:
            reason = 'No feasible allocation in the one-or-two-supplier, exact-quantity search. Check early shortages, MOQ, capacity, urgent deadlines and supplier thresholds. Consider expediting stock or changing the inputs.'
