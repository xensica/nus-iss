...
def brief(analysis, post=None):
    ...
    evidence={pid:dict(quantity=r['forecast']['quantity'] if r['forecast'] else None,
        ...
        allocations=r['plan'].get('orders',[]),
        event_scenario=r.get('overrides',{}).get('event_evidence'),
        assumed_uplift_percent=r['config']['uplift'] if r.get('overrides',{}).get('event_evidence') else None) for pid,r in analysis['results'].items()}
